    ...
    * */
    ...
    public void testSqlQuery () throws IOException {
        ...
        Pattern queryPattern = Pattern.compile("SELECT\\s+\\*\\s+FROM\\s+skill" +
                "\\s*(LEFT|INNER)?\\s+JOIN\\s+jobs_skills\\s+ON\\s+(skill.id\\s+=\\s+jobs_skills.skill_id|jobs_skills.skill_id\\s+=\\s+skill.id)" +
                "(\\s*WHERE\\s+jobs_skills.job_id\\s+IS\\s+NOT\\s+NULL)?" +
